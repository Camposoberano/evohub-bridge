...
-- Projeto 1: orquestracao multi-LLM com fallback, revisao cruzada e auditoria.

-- Enums especificos do roteador
do $$ begin
...
-- Cadastro de modelos disponiveis para o roteador
...
-- Fila/controle de tarefas do orquestrador
...
-- Historico de cada tentativa em cada modelo
...
-- Pacote de handoff para troca de modelo sem perda de contexto
...
-- Resultado de quality gates por tentativa
...
-- Seeds basicos alinhados ao plano inicial (podem ser ajustados depois)
...
-- RLS: authenticated pode ler monitoramento. Escrita fica na service_role.
...
create policy llm_tasks_read on llm_tasks
  for select to authenticated using (true);

drop policy if exists llm_runs_read on llm_runs;
create policy llm_runs_read on llm_runs
  for select to authenticated using (true);

drop policy if exists llm_handoffs_read on llm_handoffs;
create policy llm_handoffs_read on llm_handoffs
  for select to authenticated using (true);

drop policy if exists llm_quality_gates_read on llm_quality_gates;
create policy llm_quality_gates_read on llm_quality_gates
  for select to authenticated using (true);
