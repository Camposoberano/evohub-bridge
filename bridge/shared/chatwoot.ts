...
const appAuthHeaders = (a: CwAcct): HeadersInit => ({
  "api_access_token": a.token,
});
const appHeaders = (a: CwAcct): HeadersInit => ({
  "api_access_token": a.token,
  "Content-Type": "application/json",
});
// criar inbox / gerenciar membros exige role Administrator -> usa adminToken se houver.
const adminHeaders = (a: CwAcct): HeadersInit => ({
  "api_access_token": a.adminToken ?? a.token,
  "Content-Type": "application/json",
});
...
export async function profileId(
  ...
  const res = await fetch(`${baseOf(acct)}/api/v1/profile`, {
...
export async function addInboxMembers(
  ...
  const res = await fetch(
    `${baseOf(acct)}/api/v1/accounts/${accountId}/inbox_members`,
    {
      method: "POST",
      headers: adminHeaders(acct),
...
export async function createApiInbox(
  name: string,
  webhookUrl: string,
  acct: CwAcct = envAcct(),
): Promise<{
  ...
  const res = await fetch(
    `${baseOf(acct)}/api/v1/accounts/${acct.accountId}/inboxes`,
    {
      method: "POST",
  ...
  if (!res.ok) {
    throw new Error(
      `Chatwoot createApiInbox ${res.status}: ${await res.text()}`,
    );
  }
  const inbox = await res.json();
  const identifier = inbox?.inbox_identifier ??
    inbox?.channel?.inbox_identifier;
  if (identifier) return { id: inbox.id, inbox_identifier: identifier };
  const got = await fetch(
    `${baseOf(acct)}/api/v1/accounts/${acct.accountId}/inboxes/${inbox.id}`,
    {
  ...
  return {
    id: inbox.id,
    inbox_identifier: full?.inbox_identifier ?? full?.channel?.inbox_identifier,
  };
}

export async function resolveInboxIdentifier(
  inboxRef: string | number | null | undefined,
  acct: CwAcct = envAcct(),
  existingIdentifier?: string | null,
): Promise<string | undefined> {
  const existing = existingIdentifier?.trim();
  if (existing) return existing;

  const raw = inboxRef == null ? "" : String(inboxRef).trim();
  ...
  if (/^\d+$/.test(raw)) {
    ...
    return (full?.inbox_identifier as string | undefined) ??
      (full?.channel as Json | undefined)?.inbox_identifier as
        | string
        | undefined;
  }
...
> {
  ...
  if (!res.ok) {
    throw new Error(`Chatwoot listInboxes ${res.status}: ${await res.text()}`);
  }
  const json = await res.json();
  const payload = (json.payload ?? []) as Json[];
  const found = payload.find((i) => i.name === name);
  if (!found) return null;
  if (found.inbox_identifier) {
    return {
      id: found.id as number,
      inbox_identifier: found.inbox_identifier as string,
    };
  }
  const got = await fetch(
    `${baseOf(acct)}/api/v1/accounts/${acct.accountId}/inboxes/${found.id}`,
    {
      headers: appHeaders(acct),
    },
  );
...
export async function setInboxWebhook(
  ...
  const res = await fetch(
    `${baseOf(acct)}/api/v1/accounts/${accountId}/inboxes/${inboxId}`,
    {
      method: "PATCH",
      headers: {
...
export async function ensureContact(
  ...
  const res = await fetch(
    `${baseOf(acct)}/public/api/v1/inboxes/${inboxIdentifier}/contacts`,
    {
      method: "POST",
  ...
  if (!res.ok) {
    throw new Error(
      `Chatwoot ensureContact ${res.status}: ${await res.text()}`,
    );
  }
  const c = await res.json();
  return {
    source_id: c.source_id,
    pubsub_token: c.pubsub_token,
    contact_id: c?.id,
  };
}
...
export async function createConversation(
  ...
  const res = await fetch(
    `${
      baseOf(acct)
    }/public/api/v1/inboxes/${inboxIdentifier}/contacts/${sourceId}/conversations`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: "{}",
  ...
  if (!res.ok) {
    throw new Error(
      `Chatwoot createConversation ${res.status}: ${await res.text()}`,
    );
  }
...
export async function createIncomingMessage(
  ...
  const res = await fetch(
    `${
      baseOf(acct)
    }/public/api/v1/inboxes/${inboxIdentifier}/contacts/${sourceId}/conversations/${conversationId}/messages`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
export async function getConversationLabels(
  ...
  const res = await fetch(
    `${
      baseOf(acct)
    }/api/v1/accounts/${acct.accountId}/conversations/${conversationId}/labels`,
    {
      headers: appHeaders(acct),
    },
  );
  if (!res.ok) {
    throw new Error(
      `Chatwoot getConversationLabels ${res.status}: ${await res.text()}`,
    );
  }
  const json = await res.json();
...
export async function setConversationLabels(
  ...
  if (!res.ok) {
    throw new Error(
      `Chatwoot setConversationLabels ${res.status}: ${await res.text()}`,
    );
  }
}
