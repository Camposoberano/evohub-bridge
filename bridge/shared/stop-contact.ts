...
/** Pago: encerra as automações normais, mas não marca o contato como não interessado. */
export async function excludePaidContact(
  db: DbClient,
  channelId: string,
  externalContactId: string,
): Promise<{ conversations: number; scheduled: number; campaigns: number }> {
  ...
  await db.from("contacts").update({
    attributes: {
      ...
      automation_excluded_reason: "pago",
      automation_excluded_at: new Date().toISOString(),
    },
  }).eq("id", contact.id);
  ...
  await db.from("events").insert({
    source: "funil",
    event_type: "contact_excluded_paid",
    channel_id: channelId,
    payload: {
      contact_id: contact.id,
      external_contact_id: phone.slice(-6),
