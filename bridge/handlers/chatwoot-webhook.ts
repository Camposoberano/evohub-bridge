...
export async function handleOutgoing(db: Db, p: Json) {
  ...
  if (
    attachments.length === 0 && /\s\[[^\]]+\s\/\s[^\]]+\]\s*$/.test(content)
  ) {
    return;
  }
  ...
  if (cwMsgId) {
    ...
    const { data: dupDelayed } = await db.from("messages").select("id,status")
      .eq("chatwoot_message_id", cwMsgId).limit(1).maybeSingle();
    if (dupDelayed && dupDelayed.status !== "failed") {
      console.log(
        "msg apareceu durante o delay defensivo — não reenvia",
        cwMsgId,
      );
      return;
    }
  ...
  if (!channel) {
    console.warn("sem canal p/ inbox", cwInboxId);
    return;
  }
  ...
  const to = (conv?.contacts as Json)?.external_contact_id as
    | string
    | undefined;
  if (!to) {
    console.warn("sem destinatário p/ conversa", cwConversationId);
    return;
  }
  ...
  if (isRyze) {
    ...
    if (attachments.length > 0) {
      for (const att of attachments) {
        ...
        if (!aUrl) {
          console.warn("anexo sem data_url", channel.id);
          continue;
        }
        const attachType = metaAttachmentType(
    ...
    if (!channel.phone_number_id) {
      console.warn("WA sem phone_number_id", channel.id);
      return;
    }
    const url = `${channel.phone_number_id}/messages`;
    const tplCmd = content.trim().match(
      /^\/(?:template|tpl|t)\s+([a-z0-9_]+)(?:\s+([a-z_]+))?$/i,
    );
    ...
    if (hybridCandidate && !hybrid) {
      console.log(
        "hybrid ignorado: destinatário LID/BSUID, usando Meta oficial",
        cwMsgId,
      );
    }
    ...
    if (!tplCmd && conv) {
      ...
      if (!win.aberta) {
        ...
        if (!hybrid) {
          ...
          const nota =
            `🚫 *JANELA ${win.tipo.toUpperCase()} FECHADA — mensagem NÃO enviada.*\n\n` +
    ...
    if (tplCmd && attachments.length === 0) {
      ...
      if (!res.ok) {
        console.error(
          "template via chat falhou:",
          JSON.stringify((res.data as Json)?.error ?? res.data).slice(0, 200),
        );
      }
      ...
      for (const att of attachments) {
        ...
        let linkUrl = aUrl;
        if (msgType === "audio") {
          const ogg = await toVoiceOgg(aUrl);
          if (ogg) {
            linkUrl = ogg;
            mediaUrl = ogg;
          }
        ...
        if (hybrid) {
          hybridRes = await hybridSendMedia(hybrid, to, linkUrl, msgType, {
            caption: (content && !captionUsed && msgType !== "audio")
              ? content
              : undefined,
            fileName: msgType === "document"
              ? ((att.fallback_title as string) ?? "arquivo")
          ...
          if (hybridRes) {
            captionUsed = captionUsed || (!!content && msgType !== "audio");
          }
        }

        if (!hybridRes) {
          const mediaPayload: Json = { link: linkUrl };
          if (content && !captionUsed && msgType !== "audio") {
            mediaPayload.caption = content;
            captionUsed = true;
          }
          if (msgType === "document") {
            mediaPayload.filename = (att.fallback_title as string) ?? "arquivo";
          }
          res = await sendMeta(token, url, {
            messaging_product: "whatsapp",
            to,
    ...
    if (!content) {
      console.warn(
        "reply comment sem texto — anexos não suportados em comments",
      );
      return;
    }
    ...
    if (!commentId) {
      console.warn(
        "reply comment: sem comment_id inbound na conversa",
        cwConversationId,
      );
      return;
    }
    ...
    for (const att of attachments) {
      ...
      res = await sendMeta(token, "me/messages", {
        ...
        message: {
          attachment: {
            type: attachType,
            payload: { url: aUrl, is_reusable: true },
          },
        },
  ...
  if (!res) {
    console.warn("nada enviado (sem conteúdo/anexo válido)", channel.id);
    return;
  }
  console.log(
    "chatwoot-out result:",
  ...
  const extractedMessageId = messages?.length
    ...
    : null;
  const extractedDataMessageId =
    d && typeof (d as { data?: unknown }).data === "object" && d.data !== null
      ? (typeof ((d.data as Json).messageId) === "string"
        ? ((d.data as Json).messageId as string)
        : null)
      : null;
  const metaId = extractedMessageId ??
    (d && typeof (d as { message_id?: unknown }).message_id === "string"
      ? (d as { message_id: string }).message_id
      : null) ??
